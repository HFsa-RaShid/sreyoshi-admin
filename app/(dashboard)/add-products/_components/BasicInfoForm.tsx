...
import { ProductStatus, ProductUnit } from "@/types/shade.interface";
...
interface BasicInfoFormProps {
  ...
  promotion: string; setPromotion: (val: string) => void;
  ...
  unit: ProductUnit; setUnit: (val: ProductUnit) => void;
  productStatus: ProductStatus; setProductStatus: (val: ProductStatus) => void;
      ...
      <div className="bg-[#F8FAFC] border border-slate-100 rounded-xl p-4 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <input type="number" required value={sellPrice} onChange={(e) => setSellPrice(e.target.value !== "" ? Number(e.target.value) : "")} className="w-full border border-slate-200 rounded-lg p-2 bg-white" />
          </div>
          <div>
            <label className="font-semibold text-slate-600 block mb-1">Old Price (৳)</label>
            <input type="number" value={regularPrice} onChange={(e) => setRegularPrice(e.target.value !== "" ? Number(e.target.value) : "")} className="w-full border border-slate-200 rounded-lg p-2 bg-white" />
          </div>
          <div>
            <label className="font-semibold text-slate-600 block mb-1">Weight/Vol</label>
            <input type="number" value={weightOrVolume} onChange={(e) => setWeightOrVolume(e.target.value !== "" ? Number(e.target.value) : "")} className="w-full border border-slate-200 rounded-lg p-2 bg-white" />
          </div>
          <div>
            <label className="font-semibold text-slate-600 block mb-1">Unit</label>
            <select value={unit} onChange={(e) => setUnit(e.target.value as ProductUnit)} className="w-full border border-slate-200 rounded-lg p-2 bg-white">
        ...
        <div className="flex items-center justify-between max-w-xs pt-2">
          ...
          <button type="button" onClick={() => setProductStatus(productStatus === "Active" ? "Inactive" : "Active")} className={`w-10 h-5 flex items-center rounded-full p-0.5 ${productStatus === "Active" ? "bg-orange-500" : "bg-slate-200"}`}>
